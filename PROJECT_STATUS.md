...
* **Core** — Entidades, Value Objects, Domain Events, contratos de repositorio (`IGenericRepository`, `IUnitOfWork`, `IFishermanRepository`).
...
* `IFishermanRepository` declarada **vacía** (sin métodos) — interfaz fantasma.
...
2. **`IFishermanRepository`** — interfaz declarada sin métodos (`FishClubAlginet.Core/Abstractions/IFishermanRepository.cs`). Es una declaración placeholder; ningún servicio la inyecta hoy.
...
* Implementar `IFishermanRepository` (métodos custom no cubiertos por el genérico) o eliminar la interfaz fantasma.
